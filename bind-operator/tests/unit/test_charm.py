...
def test_start_without_peer_relation(context, base_state):
    ...
    assert out.unit_status == ops.testing.WaitingStatus("Topology is not available")
...
def test_dns_record_relation_changed_without_conflict(context, base_state):
    ...
    dns_record_relation = ops.testing.Relation(
        endpoint="dns-record",
    ...
    # Only ops.testing if the unit has an Active status without checking the status description
...
def test_dns_transfer_relation_changed(context, base_state):
    ...
    dns_transfer_relation = ops.testing.Relation(
